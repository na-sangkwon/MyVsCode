...
def show_update_preview(data, before_day, user_settings):
    ...
    if user_settings['carrot']:
        # [정정] 미등록의뢰는 "의뢰는 등록됐는데 그 플랫폼 매물번호가 아직 없다"는 뜻이라
        # 당근에도 똑같이 있는 개념이다(테스트 모드에서 obangData['당근_미등록_건수']로 이미
        # 계산해 둔 값, 691813처럼 당근 광고가 없는 경우) — "당근엔 없는 개념"이라고 판단한 게
        # 틀렸다. 관심수정(즐겨찾기 분류)만 당근 처리 로직에 없는 개념이라 "해당없음"으로 남긴다.
        해당없음 = "— (해당없음)"
        tree.insert("", "end", values=(
            "당근부동산",
            data.get('당근_신규등록', 0) if 업데이트_실행됨 else 제외표시,
            data.get('당근_금일등록', 0) if 업데이트_실행됨 else 제외표시,
            data.get('당근_미등록_건수', 0) if 업데이트_실행됨 else 제외표시,
            해당없음,
...
def run_platform_workers(obangData, target_mode, user_settings, progress_callback, unattended=False):
    ...
    try:
        ...
        if user_settings['carrot']:
            ...
            # [버그 수정] 총건수 계산에는 cc(최종완료_개수)를 넣어놓고, 정작 화면 문구에는 "성공"
            # 라벨이 없어서 cc가 어디서도 안 보였다 — "(총1건) 끌올:0 수정:0 비공개:0 건너뜀:0
            # 실패:0 미등록:0"처럼 표시된 항목을 다 더해도 0인데 총건수만 1로 나와 검산이 안 되는
            # 상황이 실제로 발생했다(사용자 지적). cc를 "성공" 라벨로 명시해 오방쪽(성공:{u_ok})과
            # 동일하게 항상 검산 가능하도록 맞춘다.
            counts['당근_요약'] = f"(총{당근_총건수}건) 성공:{cc} 끌올:{ro + ho}(일반{ro}/숨김해제{ho}) 수정:{uo} 비공개:{eo} 건너뜀:{so} 실패:{err_c} 미등록:{당근_미등록_건수}"
            progress_callback(
                'carrot', 100, 100,
                f"✅ 당근 업데이트 완료 V \n(총{당근_총건수}건) 성공:{cc} 끌올:{ro + ho}(일반{ro}/숨김해제{ho}) 수정:{uo} 비공개:{eo} 건너뜀:{so} 실패:{err_c} 미등록:{당근_미등록_건수}",
                'determinate'
            )

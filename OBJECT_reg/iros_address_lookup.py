...
# 크롬확장이 아니라 로컬도우미가 직접 스폰하는 독립된 셀레니움 크롬에서 처음부터 끝까지 최소화한
# 채로 실행한다(2026-09-06 실측 검증 — Phase 1에서 토지·집합건물 두 실제 매물로 확인 완료).
#
...
def find_register_address(payload):
    ...
    try:
        ...
        # [검증됨 — Phase 1] 이 흐름은 로그인이 필요 없어서 처음부터 끝까지 최소화해도 문제없다.

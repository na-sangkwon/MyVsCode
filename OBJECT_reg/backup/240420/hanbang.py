...
def macro(data, user):
    ...
    def 기본주소정보등록():
        ...
        if obinfo_type != '토지':
            ...
            if object_type != '주거용':
                ...
                라벨텍스트로span입력값넣기(driver, '룸 수', '9')
                라벨텍스트로span입력값넣기(driver, '욕실 수', '4')
    ...
    def 메모기타정보등록():
        ...
        detail = ''
        detail += Keys.ENTER + '[☞한방 매물번호: ' + object_code + ']' + Keys.ENTER
        if object_detail != '':
    ...
    def 신규매물등록():
        ...
        # pyautogui.alert("확인필요")  
        금액옵션정보등록()
        # if obinfo_type == "주거용" and obinfo_ttype != "매매":

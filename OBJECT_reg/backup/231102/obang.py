...
def macro(data, user):
    ...
    if obang_code == '' :
        ...
        if re.match('^[0-9-]+$', data['landData'][0]['land_jibun']) or re.match('^산[0-9-]+$', data['landData'][0]['land_jibun']):
            ...
            if object_type == '주거용' and tr_target == '층호수':
                driver.find_element(By.XPATH, '//*[@id="keyword"]').send_keys(location_room[:-1]) #호실명의 마지막글자를 제거한 문자를 검색어로 입력
    ...
    # 기본정보
            ...
            object_detail += (Keys.ENTER + f'● 건물옵션:{building_options}') if (building_options != '' and tr_target != '토지') else ''
            object_detail += (Keys.ENTER + f'● 호실옵션:{room_options}') if (room_options != '' and tr_target == '층호수') else ''

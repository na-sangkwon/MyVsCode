...
def remove_existing_files(ftp, directory):
    ...
    file_list = ftp.nlst(directory) #디렉토리내 파일 및 서브디렉토리 리스트가져오기
...
def macro(data, user):
    ...
    if tr_target == '층호수':
        if data['writeData']['object_type'] == '주거용':
            ...
            obinfo_type = '상가/사무실'
        elif data['writeData']['object_type'] == '공업용':
            obinfo_type = '공장/창고'
    elif tr_target == '건물':
        obinfo_type = '통건물'
    elif tr_target == '토지':
    ...
    # 기본정보
            ...
            if len(변환폴더생성일모음)>0:
                try:
                    ...
                    ftp_directory = 'img/web/object/object_img/'+object_code_new
                    ...
                    with FTP('obangkr.cafe24.com', 'obangkr', 'Ddhqkd!1') as ftp:
                        if not is_directory_exists(ftp, ftp_directory):
                            ...
                            #     print(f"{ftp_directory} 디렉토리가 존재합니다.")
                            # except Exception as e:

                            #     print(f"{ftp_directory} 디렉토리가 존재하지 않습니다. 에러 메시지: {str(e)}")
                    ...
                    for photo in photo_list:
                        ...
                        query = f'''INSERT INTO pr_object_img (object_code_new,output_folder,oimg_name,oimg_index,oimg_wdate,oimg_wtime,oimg_del) 
                                        VALUES ("{object_code_new}","{max(변환폴더생성일모음)}","{photo}","{index}","{formatted_date}","{formatted_time}","N")'''
                        cursor.execute(query)
        ...
        try:
            ...
            if len(변환폴더생성일모음)>0:
                try:
                    ...
                    with FTP('obangkr.cafe24.com', 'obangkr', 'Ddhqkd!1') as ftp:
                        if not is_directory_exists(ftp, ftp_directory):
                            print("수정시 디렉토리생성:", ftp_directory)  
                            # # 디렉토리 존재 여부 확인
                            # ftp.cwd('/')  # 루트 디렉토리로 이동
                            # current_directory = ftp.pwd()
                            # print(f"현재 작업 디렉토리: {current_directory}")                        

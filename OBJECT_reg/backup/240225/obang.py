        ...
        if object_type == '주거용':
            if data['roomData']['room_rcount'] == '':
                ...
                if room_rcount >= 1 and room_rcount < 2:
                    ...
                    if room_rcount == 1:
                        obinfo_type2 = '오픈형'
                    elif room_rcount == 1.5:
                        obinfo_type2 = '1.5룸'
                    elif room_rcount == 1.8:
                        obinfo_type2 = '1.8룸'
                elif room_rcount >= 2:
                    obinfo_type = '투룸/쓰리룸+'
                    if room_rcount == 2:
...
#################
            ...
            if tr_target != '토지':
                ...
                updated_options = [replace_options.get(option.strip(), option.strip()) for option in main_options]
...
#################            
            ...
            if obinfo_trading != '':
                object_detail += '<p><br>' + f'● 매매금액: {숫자한글로금액변환(obinfo_trading)}</p>' 
                if sum_deposit == '':
                    print("보증금이 공백입니다.")
                else:
                    print("보증금이 존재합니다.")
                object_detail += ('<p><br>' + f'● 총보증금: {숫자한글로금액변환(sum_deposit)}</p>') if str(sum_deposit) != '' else '' 
                if sum_rent != '':
                    object_detail += ('<p><br>' + f'● 총월세: {숫자한글로금액변환(sum_rent)}</p>') if str(sum_rent) != '' else ''             
                    
            elif obinfo_deposit1 != '':
                object_detail += '<p><br>' + f'● 보증금: {obinfo_deposit1}만원</p>' 
                if obinfo_rent1 != '':
                    object_detail += '<p><br>' + f'● 월세: {obinfo_rent1}만원</p>'
                if basic_manager == '별도' and float(basic_mmoney) > 0:
                    object_detail += '<p><br>' + f'● 관리비: {basic_mmoney}만원</p>'
                # if premium_exist == '있음' & premium > 0:
                #     object_detail += f'● 권리금: {premium}만원'
            if object_type == '주거용' and tr_target == '층호수':
                object_detail += (('<p><br>' + f'● 방: {int(float(basic_rcount))}개')+(f' / 욕실:{basic_bcount}개</p>' if float(basic_rcount) > 0 else '')) if float(basic_rcount) > 0 else ''
            else:
                if tr_target == '건물':
                    object_detail += ('<p><br>' + f'● 총층: {str(building_grndflr-building_ugrndflr)}층 (지상{str(building_grndflr)}층 / 지하{str(building_ugrndflr)}층)</p>') if int(building_grndflr-building_ugrndflr) > 0 else ''
                elif tr_target == '층호수':
                    if main_area != '':
                        object_detail += ('<p><br>' + f'● 면적: {main_area}㎡ (약{main_area_pyeong}평)</p>') if float(main_area) > 0 else ''
            
            object_detail += ('<p><br>' + f'● 건물옵션:{building_options}</p>') if (building_options != '' and tr_target != '토지') else ''
            object_detail += ('<p><br>' + f'● 호실옵션:{room_options}</p>') if (room_options != '' and tr_target == '층호수') else ''
            object_detail += '<p><br>' + f'● 위치: </p>'
            
            object_detail += '<p><br>' + '<br>' + '[ 매 물 주 요 특 징 ]</p>'
            object_detail += '<p><br>' + 'ㅇ </p>'
            object_detail += '<p><br>' + 'ㅇ </p>'
            ...
            # detail += '<br>' + '📋상세정보'
            detail += '<p><br>' + object_detail + '<br></p>'
            detail += '<p><br>' + '----------------------------------------------------------------------------------------------</p>'
            detail += '<p><br>' + '◈아직 등록되지 않은 매물도 다수 보유중이니 더 많은 매물을 안내받길 원하신다면 문의주시기 바랍니다.</p>'
            detail += '<p><br>' + '◈편하게 연락 주시고 홈페이지도 방문해보세요!!</p>'
            detail += '<p><br>' + '※렌트프리, 옵션, 협의사항 등 끝까지 도와드리겠습니다.</p>'
            # detail += '<p><br>' + '📞010-8631-4392'
            # detail += '<p><br>' + '📌홈페이지: osanbang.com'
            detail += '<p><br>' + '----------------------------------------------------------------------------------------------' + '<br></p>'
            # iframe으로 스위치

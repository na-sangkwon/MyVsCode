...
def macro(data, user):
    ...
    if response == "확인":
        # 다음 코드 실행
        print("사용자가 확인을 눌렀습니다. 다음 코드를 실행합니다.")
        # 다음 로직 작성
    else:
        print("사용자가 취소를 눌렀거나 창을 닫았습니다.")
    
    pyautogui.alert("테스트 완료")
    ...
    master_data = data['masterData']['master_data']
    print("master_data:",master_data)
    master_keys_list = list(master_data.keys())
    # 리스트에 요소가 있는지 확인하고 첫 번째 요소에 접근합니다.
    if master_keys_list:  # 리스트가 비어있지 않은 경우
        master_info = master_data[master_keys_list[0]]
    else:
        # master_keys_list가 비어 있으면 적절한 처리를 합니다.
        print("master_keys_list: No data available")
        master_info = None  # 혹은 다른 기본값 할당
    ...
    등록된소유자들 = data['writeData']['master_name']
    ...
    등기부확인여부 = data['writeData']['master_check']
    ...
    if tr_target == '건물' or tr_target == '층호수':
        if 'brtitData' in data and isinstance(data['brtitData'], dict):
            # brtitData가 딕셔너리일 경우
            brtit_platPlc = data['brtitData'].get('brtit_platPlc', '')
            brtit_bldNm = data['brtitData'].get('brtit_bldNm', '')
            brtit_dongNm = data['brtitData'].get('brtit_dongNm', '')
            brtit_bldNmdongNm = data['brtitData'].get('brtit_bldNmdongNm', '')
    ...
    if tr_target == '층호수':
        ...
        room_memo = '' if data['roomData']['room_memo'] == '' else data['roomData']['room_memo'] + "\n"
    ...
    #             # EC.element_to_be_clickable((By.XPATH, f"//div[@class='v-overlay-container']//div[text()='{선택항목}']")) #일치
    #             EC.element_to_be_clickable((By.XPATH, f"//div[@class='v-overlay-container']//div[contains(text(), '{선택항목}')]")) #포함
    #         )
    #         time.sleep(0.3)
    #         선택항목요소.click()
    #         print(f"선택완료:{선택항목}")
    #     except Exception as e:
    #         print(f"An error occurred: {e}")
    #         pyautogui.alert(f"{선택항목}을 찾을 수 없습니다. 직접 선택하세요!!")
    ...
    def 홍보확인서선택및의뢰인정보입력(fail_msg):   
        ...
        if 등기부확인여부=='Y' and len(등록된소유자들_arr) > 0: #등기확인된 소유주
            ...
            if master_info and 'master_name' in master_info: #본인 또는 대표인 접촉자정보존재
                ...
                elif master_info['master_name'] not in 등록된소유자들 and master_info['master_type'] == '대표':
                    print("5")
                    소유자명 = master_name
                else:
    ...
    # 기등록 네이버매물번호가 존재여부확인
    네이버매물번호 = data['adData']['네이버'].get('ad_code', '') #키가 존재하지 않을 경우 기본값을 반환
    네이버메모 = data.get('adData', {}).get('네이버', {}).get('memo', '')
    
    써브뒤숫자 = re.search(r'써브:(\d+)', 네이버메모)# "써브:" 뒤의 숫자 추출
    써브매물번호 = 써브뒤숫자.group(1) if 써브뒤숫자 else ''  # 숫자를 추출하거나 빈 문자열 반환
    네이버종료일 = data['adData']['네이버'].get('ad_end', '')
    # pyautogui.alert("네이버매물번호:"+네이버매물번호+" 네이버종료일:"+네이버종료일)
    ...
    #기본정보
        ...
        if obinfo_type1 == '':
            ...
            try:
                ...
                #대분류 
                특정위치X번째셀렉트에서선택('매물 분류', 2, obinfo_type2)
    ...
    #관리비 부과정보
        ...
        if tr_target != '토지': 
            if basic_manager == '별도' and obinfo_type1 != '토지':
                ...
                if obinfo_type1 in ['아파트', '오피스텔','주택']:
                    라디오버튼선택('부과방식', '정액관리비 (세부내역 미고지한 경우)')
                    라디오버튼선택('부과기준', '직전 월 관리비')
                    특정위치X번째셀렉트에서선택('관리비 타입',1,'정액관리비이지만 중개의뢰인이 세부내역 미고지한 경우')
                    ...
                    if obinfo_ttype=='매매':
                        ...
                        if basic_manager == '없음':
                            if obinfo_type1 in ['오피스텔','원룸']:
                                라디오버튼선택('부과방식', '확인불가')
                                관리비없음체크박스 = 특정위치의x번째입력태그찾기('확인불가 사유', 'checkbox', 1)
                                # pyautogui.alert("go?")
                                # 관리비없음체크박스.click()
                            else:
                                관리비없음체크박스 = 특정위치의x번째입력태그찾기('월 관리비', 'checkbox', 1)
    ...
    # 기등록 네이버매물번호 존재
        ...
        #광고 종료일이 경과된 상태인지 확     
        print("써브매물번호:"+써브매물번호," 네이버종료일:"+네이버종료일)
        # 종료일 경과 여부 확인
        if 네이버종료일:
        ...
        # try:
        #     # 전체건수가 1 또는 0인 경우에 따른 처리
        #     검색결과건수요소 = WebDriverWait(driver, 10).until(
        #         EC.presence_of_element_located((By.XPATH, '//div[@class="total-area"]//span'))
        #     )
        #     검색결과건수 = 검색결과건수요소.text.strip()            
        #     print("검색결과건수:"+str(검색결과건수))
        #     # pyautogui.alert("검색결과건수:"+str(검색결과건수))
        # except Exception as e:
        #     print("검색결과건수를 확인하는 데 실패했습니다.", e)      
        #     # pyautogui.alert("검색결과건수를 확인하는 데 실패했습니다.")
...
#물건사진 폴더열기
    ...
    # response = pyautogui.confirm(
    #     text="{}\n{}\n\n매물번호를 추출하여 저장합니다.\n\n수정내용 저장후\n써브/네이버/KB부동산 매물번호가 보이는 페이지에서\n'확인'버튼을 눌러주세요".format(address_info, fail_msg),
    #     title="[네이버부동산]",
    #     buttons=["확인", "취소"]
    # )
    ...
    driver.get('https://ma.serve.co.kr/good/articleTrsmDetail?atclNo=316991171')
    try:
        # admin_id = "상가팀원"
        # object_code_new = "743186"
        # 매물번호를 저장할 변수 초기화
        네이버매물번호 = ""
        써브매물번호 = ""
        KB매물번호 = ""
        ...
        try:
            ...
            if existing_record:
                ...
                if current_ad_code == 네이버매물번호:
                    ...
                    # 쿼리에 사용할 변수 생성
                    query_params = [네이버매물번호, current_date, current_time, ad_memo, 써브매물번호]
                ...
                try:
                    ...
                    cursor.execute(insert_query, (
                        admin_id, object_code_new, ad_start, ad_end, '네이버', 네이버매물번호, current_date, current_time, ad_memo, 써브매물번호, current_date, current_time
                    ))
                    if admin_id and object_code_new and 네이버매물번호 and 써브매물번호 :
                        conn.commit()
                        alert_message = f"새 네이버 광고 매물이 추가되었습니다.\n\부동산써브: {써브매물번호}\네이버부동산: {네이버매물번호}\KB부동산: {KB매물번호}"
                except Exception as e:
                    alert_message = f"추가 작업 중 오류가 발생했습니다: {e}"

            # 알림창으로 결과 표시
            if alert_message : pyautogui.alert(alert_message)

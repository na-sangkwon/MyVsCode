...
def macro(data, user):
    ...
    if tr_target == '층호수':
        ...
        room_memo = data['roomData']['room_memo'].decode('utf-8') #호실메모
...
#위치정보
        ...
        # 카테고리별_텍스트_찾기('매물 공개 여부', 'checkbox', '공실')
        # 카테고리별_텍스트_찾기('매물 종류', 'radio', '쓰리룸+')
...
#기본정보
        ...
        if tr_target == '층호수':
            카테고리별_텍스트_찾기('roombath1', '방', 'number', room_rcount) #방수
            카테고리별_텍스트_찾기('roombath2', '욕실', 'number', room_bcount) #욕실수
...
#상세정보
        ...
        if object_type == '주거용' and tr_target == '층호수':
            ...
            if tr_target == '건물':
                ...
                object_detail += ('<p>' + f'● 대지면적: {land_totarea}㎡ (약{main_area_pyeong}평)' + '</p>') if float(main_area) > 0 else ''
                # pyautogui.alert(representing_jimok)
        ...
        object_detail += ('<p>' + f'ㅇ {land_memo}' + '</p>') if land_memo else ''
        if tr_target != '토지' : object_detail += ('<p>' + f'ㅇ {building_memo}' + '</p>') if building_memo else ''
        if tr_target == '층호수' : object_detail += ('<p>' + f'ㅇ {room_memo}' + '</p>') if room_memo else ''
        object_detail += '<p>' + 'ㅇ ' + '</p>'
        object_detail += '<p>' + 'ㅇ ' + '</p>'

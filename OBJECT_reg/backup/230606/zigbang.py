...
def macro(data, user):
    ...
    if building_parking == '있음' : 
        ...
        # driver.find_element(By.XPATH, '//*[@id="item_form"]/section[4]/div/div[2]/div[2]/div/div[1]/div/label[1]/span').click()
        # WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="item_form"]/section[4]/div/div[1]/div[2]/div/div[1]/div/label[1]'))).click()
    else: 
        driver.find_element(By.CSS_SELECTOR, '#item_form > section:nth-child(10) > div > div:nth-child(1) > div.sc-gipzik.bEek > div > div:nth-child(1) > div > label:nth-child(2) > span').click()
    
    #엘리베이터
    if '엘리베이터' in building_option:
        print("건물옵션에 엘리베이터 포함")
        driver.find_element(By.CSS_SELECTOR, '#item_form > section:nth-child(10) > div > div:nth-child(2) > div.sc-gipzik.bEek > div > label:nth-child(1) > span').click()
    # pyautogui.alert('ㄱㄱ?')
    #관리비
    if manager == '별도': 

...
def getData(object_code_new, userid, userpw):
    ...
    if num_rows == 0:
        ...
        #소유주정보
        query = 'SELECT * FROM pr_contactor AS a INNER JOIN pr_client AS b ON a.client_code=b.client_code WHERE a.contactor_del="N" AND a.contactor_type="본인" AND a.request_type="내놓기" AND a.request_code = "%s"' % request_code
        cursor.execute(query)

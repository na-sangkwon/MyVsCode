...
def macro(data, user):
    ...
    # pyautogui.alert("master_data 확인")
    ...
    def 라벨들로체크박스클릭(strong태그의텍스트, 체크박스_라벨들):
        ...
        try:
            ...
            for strong in strong_elements:
                ...
                # 해당 tr 내의 체크박스들 찾기
                checkboxes = tr_element.find_elements(By.XPATH, './/input[@type="checkbox"]')
                labels = tr_element.find_elements(By.XPATH, './/label')
    ...
    # 특정tr요소('등기부상 소유자 이름')
    # 특정위치의x번째입력태그찾기('상세정보', 'textarea', 1).send_keys(description) #느림
    # pyautogui.alert("go?")
...
#기본정보
    ...
    # # 모든 strong 태그의 텍스트를 출력하며, 보이는 상태인지 여부도 함께 표시
    # all_strong_elements = driver.find_elements(By.XPATH, '//th/strong')
    # print(f"Found {len(all_strong_elements)} total strong elements:")
    # for strong in all_strong_elements:
    #     is_displayed = strong.is_displayed()
    #     if is_displayed:
    #         # print(f"Strong element text: {strong.text}, Is displayed: {is_displayed}")
    #         tr_element = strong.find_element(By.XPATH, './ancestor::tr')
    #         td_elements = tr_element.find_elements(By.XPATH, './td')
    #         if strong.text == '등기부상\n소유자 이름':
    #             for td in td_elements:
    #                 elements = td.find_elements(By.XPATH, './/input[@type="text"]')
    #                 for elem in elements:
    #                     print(f"Found {strong.text} element: {elem.get_attribute('outerHTML')}")  # 디버깅 출력
    #                     elem.send_keys('홍길동')


    # 특정tr요소('등기부상 소유자 이름')
    # 특정위치의x번째입력태그찾기('등기부상 소유자 이름', 'text', 1).send_keys('홍길동') 
    # 특정위치의x번째입력태그찾기('보증금', 'Number', 1)
    # pyautogui.alert("go?")
#매물소재지
...
#매물정보
    ...
    if obinfo_type1 in ['원룸','주택','상가점포','사무실']:
        print('basic_area1:'+str(basic_area1))
        basic_area1 = '20' if (basic_area1=='' or basic_area1=='') else basic_area1
    ...
    if obinfo_type1 in ['원룸','주택']:
        ...
        room_memo += "\n"+"-- 방수: "+basic_rcount+" / 욕실수: "+basic_bcount
    #방향
...
#원룸/투룸 방찾기
    ...
    if obinfo_type1 in ['원룸']:
        if '오픈형' in room_important:
            ...
            라디오버튼선택('방 구조(타입)', '분리형')
    #방특징
        라벨들로체크박스클릭('방 특징', room_important)
#시설정보
    if obinfo_type1 != '토지':
        # print("optionImportant:"+optionImportant)
        시설정보항목들 = 그룹별명칭변환('시설정보', optionImportant)
...
#의뢰인정보
    # pyautogui.alert("정상?")   
    if client_info and 'client_name' in client_info:
        소유자명 = client_info['client_name'] if '이름미확인' not in client_info['client_name'] else ''
        소유자연락처 = client_info['client_phone1']
    else:
        소유자명 = ''
        소유자연락처 = ''
    특정위치의x번째입력태그찾기('등기부상 소유자 이름', 'text', 1).send_keys(소유자명) 
    # pyautogui.alert("정상?")   
    특정위치의x번째입력태그찾기('의뢰인과 등기부상 소유자와의 관계', 'text', 1).send_keys('본인') 
    if len(소유자연락처) == 11:

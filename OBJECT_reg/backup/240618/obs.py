...
def macro(data, user):
    ...
    def 카테고리별_텍스트_찾기(DIV영역클래스명, 카테고리_이름, 입력_타입, 대상텍스트):
        ...
        try:
            ...
            print(f'카테고리({보이는_카테고리_spans})의 개수' + str(len(보이는_카테고리_spans)))
...
#         pyautogui.alert("계속진행??")
        ...
        if tr_target == '층호수' and room_status == '공실' :
            ...
            카테고리별_텍스트_찾기('detail_title', '매물 종류', 'radio', '통건물') #1차그룹 통건물 클릭
        ...
        if tr_target == '층호수':
            ...
            clickList('#form_basic_info > div.form_table > div:nth-child(7) > div.form_table_box.heat > div.form_table_box_cnt > div.select_boxs.select_box_item_heat', '개별') #난방종류
            #입주가능일
            if room_status == '공실':

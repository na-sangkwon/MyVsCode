...
def macro(data, user):
    ...
    basic_mmoney = '' if data['writeData']['mmoney']=='' else float(data['writeData']['mmoney'])*10000 #관리비
    ...
    if tr_target == '건물' or tr_target == '층호수':
        brtit_bldNm = data['brtitData']['brtit_bldNm']
        brtit_dongNm = data['brtitData']['brtit_dongNm']
        brtit_bldNmdongNm = data['brtitData']['brtit_bldNmdongNm']
        ...
        세대당주차대수값 = (building_pn/int(building_hhld)) if building_pn and building_hhld else 0
    ...
    if tr_target == '층호수':
        ...
        basic_secret += room_info
        
    basic_secret += " " + client_info + "\n" + formatted_date+" "+admin_name
    ...
    # pyautogui.alert("brtit_dongNm:"+brtit_dongNm)
    ...
    def 셀렉트항목선택(선택항목, 최상위요소div의XPATH):
        print(f"셀렉트항목선택:{선택항목}")
        try:
            최상위요소div = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, f'{최상위요소div의XPATH}'))
            )
            print("최상위요소div 찾음")
            최상위요소div.click()
            ...
            for strong in strong_elements:
                ...
                if strong태그명 == strong_text:
                    ...
                    for td in td_elements:
                        ...
                        # print(f"elements 개수: {len(target_divs)}개")
                        # pyautogui.alert("check!!")
                        if X번째 <= len(target_divs):
                            target_div = target_divs[X번째 - 1]  # X번째 요소 선택 (0부터 시작하므로 -1)
    ...
    # 기등록 네이버매물번호가 존재여부확인
    네이버매물번호 = data['adData']['네이버'] if data['adData']['네이버'] != '' else ''
    ...
    if 네이버매물번호 == '':
        ...
        #         print(f"Found div_element태그: {div_element.get_attribute('outerHTML')}")  
        #         # 해당 div 내의 td의 첫 번째 div 내에 있는 label 태그 찾기
        #         label_elements = div_element.find_elements(By.XPATH, './/div/div/label')
        #         print(f"label_elements")    
        #         # 각 label 태그의 텍스트 출력
        #         for label in label_elements:
        #             print(f"Label 텍스트: {label.text}") 
        #             if label.text=='모바일확인V2 (집주인)':
        #                 label.click()     
        #                 print(f"검증방식({label.text}) 클릭완료")    
        #     #네이버등록권
        #     특정위치의x번째입력태그찾기('네이버등록권', 'radio', 1).click()
        #     pyautogui.alert("등기부상 소유자 이름 display상태?")
        # except:
        #     fail_msg = '\n- 네이버등록권 선택실패'
        #     pyautogui.alert("오류?")
        # # time.sleep(5)
    ...
    #기본정보
        ...
        if obinfo_type1 == '':
            ...
            try:
                #소분류
                셀렉트항목선택(obinfo_type1, '//*[@id="app"]/div/div/div[3]/div/div[1]/div[4]/table/tbody/tr[1]/td/div[1]/div[1]')
                #대분류
                셀렉트항목선택(obinfo_type2, '//*[@id="app"]/div/div/div[3]/div/div[1]/div[4]/table/tbody/tr[1]/td/div[1]/div[2]')
        ...
        # time.sleep(3)
        # 셀렉트항목선택('개별난방', '//*[@id="app"]/div/div/div[3]/div/div[1]/div[13]/table/tbody/tr[1]/td/div')
        # 특정위치X번째셀렉트에서선택('건축물용도', 1, '위락시설')
    ...
    #매물소재지
        ...
        # 셀렉트항목선택(그룹별명칭변환('지역(시/도)', location_do), '//*[@id="app"]/div/div/div[3]/div/div[1]/div[5]/table/tbody/tr[1]/td/div[1]/div/div[1]')
        # 셀렉트항목선택(location_si, '//*[@id="app"]/div/div/div[3]/div/div[1]/div[5]/table/tbody/tr[1]/td/div[1]/div/div[2]')
        # 셀렉트항목선택(location_dong, '//*[@id="app"]/div/div/div[3]/div/div[1]/div[5]/table/tbody/tr[1]/td/div[1]/div/div[3]')
        if location_li != '': 
        ...
        if obinfo_type1 in ['아파트','오피스텔']:
            ...
            if obinfo_type1 == '아파트':
                단지선택항목 = brtit_bldNm
                셀렉트항목선택(단지선택항목, '//*[@id="app"]/div/div/div[3]/div/div[1]/div[5]/table/tbody/tr[2]/td/div[1]/div[1]')
    ...
    #가격정보
        ...
        if obinfo_type1 in ['상가점포','사무실']:
            if premium_exist == '있음':
                ...
                premium_memo = "\n"+"-- 권리금(시설비) : " + ((한글금액(premium) + " 등") if premium.isdigit() else premium)
    ...
    #관리비 부과정보
        ...
        if basic_manager == '별도' and obinfo_type1 != '토지':
            ...
            if obinfo_type1 in ['원룸','주택']:
                ...
                특정위치의x번째입력태그찾기('월 관리비', 'number', 1).send_keys(basic_mmoney)
    ...
    #매물정보
        ...
        if obinfo_type1 in ['아파트']:
            ...
            if brtit_dongNm.endswith('동'):
                # 마지막 '동' 제거
                brtit_dongNm = brtit_dongNm[:-1]
        ...
        # pyautogui.alert("정상?") 
        if obinfo_type1 in ['아파트', '사무실', '상가점포']:
            #해당층 #해당동 총층
            if obinfo_type1 in ['아파트']:
                특정위치의x번째입력태그찾기('해당층 / 해당동 총층', 'text', 1).send_keys(basic_floor)
                특정위치의x번째입력태그찾기('해당층 / 해당동 총층', 'number', 1).send_keys(basic_totflr)
            elif obinfo_type1 in ['상가점포','사무실']:
                특정위치의x번째입력태그찾기('해당층 / (해당동) 총층', 'text', 1).send_keys(basic_floor)
                특정위치의x번째입력태그찾기('해당층 / (해당동) 총층', 'number', 1).send_keys(basic_totflr)
        # elif obinfo_type1 in ['상가점포']:
        #     특정위치의x번째입력태그찾기('해당층 / (해당동) 총층', 'text', 1).send_keys(basic_floor)
        # pyautogui.alert("정상?")      
        ...
        if tr_target=='건물':
            #해당층
                ...
                특정위치의x번째입력태그찾기('지상 / 지하 총층', 'number', 1).send_keys(building_grndflr)
                if building_grndflr: room_memo += "\n"+"-- 지상층: "+building_grndflr+"층"
            # pyautogui.alert("정상?")   
            #총층
                print('building_ugrndflr:'+str(building_ugrndflr))
                특정위치의x번째입력태그찾기('지상 / 지하 총층', 'number', 2).send_keys(building_ugrndflr)
                if building_ugrndflr: room_memo += "/지하층: "+building_ugrndflr+"층"
        elif tr_target=='층호수':
            #해당층
        ...
        #방향기준/방향
        if obinfo_type1 in ['원룸','주택','아파트']:
            셀렉트항목선택('안방', '//*[@id="app"]/div/div/div[3]/div/div[1]/div[11]/table/tbody/tr[12]/td/div[1]/div[1]')
            if room_direction: 셀렉트항목선택(room_direction, '//*[@id="app"]/div/div/div[3]/div/div[1]/div[11]/table/tbody/tr[12]/td/div[1]/div[2]')
        ...
        #주차가능여부
        if obinfo_type1 in ['원룸','주택','아파트','상가점포','사무실']:
            ...
            if obinfo_type1 == '아파트':
                print('세대당 주차대수 미입력')
                # #세대당 
                # 특정위치의x번째입력태그찾기('세대당 / 총 주차대수', 'text', 1).send_keys(세대당주차대수값)
                # #총
                # 특정위치의x번째입력태그찾기('세대당 / 총 주차대수', 'number', 1).send_keys(building_pn)
            else:
        ...
        if obinfo_type1 in ['원룸','주택','상가점포','사무실']:
            ...
            if building_usedate == '0000-00-00':
                ...
                # 셀렉트항목선택('사용승인일', '//*[@id="app"]/div/div/div[3]/div/div[1]/div[11]/table/tbody/tr[31]/td/div[1]/div[1]')
                # 셀렉트항목선택(usedate[0], '//*[@id="app"]/div/div/div[3]/div/div[1]/div[11]/table/tbody/tr[31]/td/div[1]/div[2]')
                # 셀렉트항목선택(usedate[1], '//*[@id="app"]/div/div/div[3]/div/div[1]/div[11]/table/tbody/tr[31]/td/div[1]/div[3]')
                # 셀렉트항목선택(usedate[2], '//*[@id="app"]/div/div/div[3]/div/div[1]/div[11]/table/tbody/tr[31]/td/div[1]/div[4]')
    #원룸/투룸 방찾기
        #방구조(타입)
        if obinfo_type1 in ['원룸','주택']:
    ...
    #시설정보
        if obinfo_type1 != '토지':
            ...
            # 셀렉트항목선택('개별난방', '//*[@id="app"]//table/tbody/tr[1]/td/div')
            #난방연료
            특정위치X번째셀렉트에서선택('난방연료', 1, '도시가스')
            # 셀렉트항목선택('도시가스', '//*[@id="app"]/div/div/div[3]/div/div[1]/div[13]/table/tbody/tr[2]/td/div')
            # #냉방시설
    ...
    #매물상세정보
        ...
        description += "" if ((room_memo + r_add_memo).strip()) == "□■ 호실정보" else (room_memo + r_add_memo)
        description += "" if building_memo.strip() == "□■ 건물정보" else building_memo
        description += "" if land_memo.strip() == "□■ 토지정보" else land_memo
        description += "" if important_memo.strip() == "□■ 주요특징" else important_memo
        # pyautogui.alert(f">>{(room_memo + r_add_memo).strip()}<<\n>>{building_memo.strip()}<<\n>>{land_memo.strip()}<<")  
        ...
        if master_check=='Y' and len(master_names) > 0: #등기확인된 소유주
            ...
            if contactor_info and 'contactor_name' in contactor_info: #본인 또는 대표인 접촉자정보존재
                ...
                print(f"소유자:{소유자명}\n연락처(통신사:{client_telecom}):{소유자연락처}")
                if 소유자연락처 and 소유자연락처 and client_telecom != '미확인':
                    선택할검증방식 = '모바일확인V2 (집주인)'
            else:
                소유자명 = master_name     
        else:
    ...
    #의뢰인정보
        ...
        if 선택할검증방식 == '홍보확인서 확인':
            ...
            if client_gender: 
                선택할성별위치 = 1 if client_gender == '남성' else 2
                특정위치의x번째입력태그찾기('등기부상 소유자 성별', 'radio', 선택할성별위치).click()
            if client_telecom:
                if client_telecom == 'SKT': 선택할통신사위치 = 1
                if client_telecom == 'KT': 선택할통신사위치 = 2
                if client_telecom == 'LGU+': 선택할통신사위치 = 3
                if client_telecom == '알)SKT': 선택할통신사위치 = 4
                if client_telecom == '알)KT': 선택할통신사위치 = 5
                if client_telecom == '알)LGU+': 선택할통신사위치 = 6
    ...
    # 기등록 네이버매물번호 존재
        ...
        # driver.implicitly_wait(10)
        # time.sleep(3)
        셀렉트항목선택('네이버', '//*[@id="app"]/div/div/div[3]/div/div[3]/table/tbody/tr[1]/td[1]/div/div[1]')
        네이버매물번호입력요소 = 특정위치의x번째입력태그찾기('매물번호', 'text', 1)
        네이버매물번호입력요소.send_keys(네이버매물번호)
        네이버매물번호입력요소.send_keys(Keys.ENTER)
        #검색된 전체건수가 1인지 확인
        # 최대 5초 동안 대기
        try:
            전체건수1 = WebDriverWait(driver, 5).until(
                lambda d: d.find_element(By.XPATH, f'//*[@id="app"]/div/div/div[3]/div/div[6]/div[1]/button/span[3]/span/em').text == '1'
            )
            print("전체건수가 1입니다!")
        except Exception as e:
            print("전체건수가 1이 되지 않았습니다.", e)
        # 1이면 재등록버튼 클릭
        # pyautogui.alert("전체건수1")
        if 전체건수1 : driver.find_element(By.XPATH, f'//*[@id="printArea"]/div/table/tbody/tr/td[7]/div/div[1]/button').click()
        time.sleep(2)
        #네이버등록권
        특정위치의x번째입력태그찾기('네이버등록권', 'radio', 1).click()  
...
#약관동의
    ...
    if agreement_label:
        ...
        if checkbox_id:
            ...
            #     print("Checkbox has been clicked via JavaScript.")
            # except Exception as e:
            #     print(f"Failed to click the checkbox via JavaScript: {e}")            
                        
            
            # # 스크롤을 시도하며 요소가 화면에 나타날 때까지 반복
            # for attempt in range(5):  # 최대 5번 시도
            #     try:
            #         # 요소가 보일 때까지 스크롤
            #         driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", checkbox_element)
            #         pyautogui.alert("정상?")
            #         # 스크롤 후, 해당 요소가 클릭 가능할 때까지 대기
            #         checkbox = WebDriverWait(driver, 3).until(
            #             EC.element_to_be_clickable((By.ID, checkbox_id))
            #         )
            #         checkbox.click()
            #         print("Checkbox has been clicked.")
            #         break  # 클릭에 성공하면 루프를 빠져나옴

            #     except :
            #         print(f"Attempt {attempt+1}: Element not clickable, scrolling further...")
            #         # 조금 더 스크롤을 내려서 다시 시도
            #         driver.execute_script("window.scrollBy(0, 300);")  # 300픽셀씩 더 내림
            #         if attempt == 4:  # 마지막 시도 후에도 실패 시
            #             print("Failed to click the checkbox after multiple attempts.")
        else:

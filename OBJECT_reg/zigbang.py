...
def macro(data, user):
    ...
    # pyautogui.alert("go??")
    # driver.find_element(By.XPATH, '/html/body/div[1]/section/div[1]/div[2]/div[2]/div/div/a[1]').click()
    ...
    # pyautogui.alert("1go??")
    # login_button = driver.find_element(By.XPATH, '//*[@id="email_로그인"]')
    # driver.execute_script("arguments[0].click();", login_button)    
    driver.find_element(By.XPATH, '//*[@id="이메일주소_인풋박스"]').send_keys("nasangkwon@outlook.kr")
    driver.find_element(By.XPATH, '//*[@id="비밀번호_인풋박스"]').send_keys("dhqkd5555%")
    # pyautogui.alert("2go??")
    driver.find_element(By.XPATH, '//*[@id="__next"]/div/div/div/section/div[2]/div').click()
    # pyautogui.alert("3go??")
    ...
    def 관리비상세지정태그(대분류, 소분류, tag_type, 몇번째):
        ...
        try: 
            ...
            소분류_text = 소분류.replace(' ', '')
            # 모든 strong 요소를 찾아낸 후에 보이는 요소만 필터링
            대분류_elements = driver.find_elements(By.XPATH, '//*[@id="react-modal"]/div/div/div[2]/div[1]/button') #
            # print(f"대분류_elements 개수: {len(대분류_elements)}개")
            # for 대분류_elem in 대분류_elements:
            #     print(f"Found 대분류태그: {대분류_text} {대분류_elem.get_attribute('outerHTML')}")
            # pyautogui.alert("확인")
            visible_대분류_elements = [elem for elem in 대분류_elements if elem.is_displayed()]
            # print(f"보이는 대분류태그 개수: {len(visible_대분류_elements)}개")
            # pyautogui.alert("확인")
            # 각 strong 태그의 부모 tr 요소 찾기
            for visible_대분류 in visible_대분류_elements:
                # HTML에서 텍스트 추출 후 모든 공백 및 줄바꿈 제거
                visible_대분류_text = visible_대분류.get_attribute('textContent').replace('\n', '').replace('\r', '').replace(' ', '')
                # print("visible_대분류_text:"+visible_대분류_text)
                # pyautogui.alert("함수 확인")
                # print(f"Found 대분류태그: {대분류_text} {visible_대분류.get_attribute('outerHTML')}")
                if visible_대분류_text == 대분류_text:
                    # section_element = 대분류.find_element(By.XPATH, './parent::section')
                    all_소분류div_elements = visible_대분류.find_elements(By.XPATH, './ancestor::div[3]//div[contains(text(), "일반(공용) 관리비") or contains(text(), "사용료") or contains(text(), "기타 관리비")]')
                    # print(f"all_소분류div_elements 개수: {len(all_소분류div_elements)}개")
    ...
    brtit_dongNm = data['brtitData']['brtit_dongNm'] if data['brtitData'] else ''
    ...
    if float(room_rcount) >= 1 and float(room_rcount) < 2:
        ...
        선택할층구조 = ''
    print('선택할층구조:', 선택할층구조)
    if 선택할층구조: selectOption('//*[@id="item_form"]/section[2]/div/div[4]/div[2]/div/div[1]/div/div/div/div/select', 선택할층구조) #층구조 선택
    ...
    if not str(building_usedate) == '0000-00-00': 지정태그('기본정보', '사용승인일', 'text', 1).send_keys(building_usedate)
    #사진
    ...
    if manager == '별도': #item_form > section:nth-child(10) > div > div:nth-child(4) > div.sc-csuQGl.hdlHHX > div:nth-child(1) > div.sc-jTzLTM.kPElNT > div > input
        ...
        if object_type =='주거용' and  float(mmoney) > 10:
            ...
            if 상세설정하기:
                ...
                관리비상세지정태그('정액관리비', '일반(공용) 관리비', 'radio', 1).click()
                관리비상세지정태그('정액관리비', '일반(공용) 관리비', 'text', 1).send_keys(str(int(float(mmoney)*10000)))
                
                print("재정의된mlist_arr:", 재정의된mlist_arr)
                for m항목 in 재정의된mlist_arr:
                    체크위치 = 0
                    if m항목 == '전기료':
                        체크위치 = 1
                    elif m항목 == '가스사용료':
                        ...
                        체크위치 = 6
                    if 체크위치 > 0:
                        관리비상세지정태그('정액관리비', '사용료', 'checkbox', 체크위치).click()
                        관리비상세지정태그('정액관리비', '사용료', 'radio', (체크위치*3)).click()
                
                관리비상세지정태그('정액관리비', '기타 관리비', 'checkbox', 1).click()
                # pyautogui.alert("저장시작!!")
                관리비기준 = driver.find_element(By.XPATH, '//*[@id="react-modal"]//strong[contains(text(), "관리비 기준을 선택해주세요.")]')
                첫번째항목 = 관리비기준.find_element(By.XPATH, './ancestor::div[1]/ul/li/label')
                첫번째항목.click()
                
                
                # pyautogui.alert("저장시작!!")
                #관리비 상세 설정하기 저장하기
                저장하기버튼 = driver.find_element(By.XPATH, '//*[@id="react-modal"]//button[contains(text(), "저장하기")]')
                저장하기버튼.click()

...
def macro(data, user):
    ...
    representing_jibun = data['landData'][0]['representing_jibun']
    ...
    if tr_target == '건물' or tr_target == '층호수':
        ...
        building_loan = data['buildingData']['building_loan'] #대출
        building_loan_rate = data['buildingData']['building_loan_rate'] #대출이자
    ...
    if len(object_code_new) != 5:
        ...
        def switch_to_nested_iframe(driver):
            """
            중첩된 iframe 구조를 명시적으로 처리하여 타겟 iframe으로 바로 전환.
            """
            try:
                # 첫 번째 iframe 전환
                outer_iframe = WebDriverWait(driver, 5).until(
                    EC.presence_of_element_located((By.XPATH, '//iframe[@title="우편번호서비스 레이어 프레임"]'))
                )
                driver.switch_to.frame(outer_iframe)
                print("우편번호서비스 레이어 프레임으로 전환 완료.")

                # 두 번째 iframe 전환
                inner_iframe = WebDriverWait(driver, 5).until(
                    EC.presence_of_element_located((By.XPATH, '//iframe[@title="우편번호 검색 프레임"]'))
                )
                driver.switch_to.frame(inner_iframe)
                print("우편번호 검색 프레임으로 전환 완료.")
                return True

            except TimeoutException:
                print("타겟 iframe을 찾을 수 없습니다.")
                return False
        ...
        try:
            # 중첩된 iframe으로 바로 전환
            if switch_to_nested_iframe(driver):
                ...
                try:
                    ...
                    search_box.send_keys(land_main+' '+representing_jibun) #"경기도 오산시 궐동 643-8"
                    ...
                    if total_count == 1:
                        ...
                        span_element = li_element.find_element(By.CLASS_NAME, "txt_addr")
                        print("span 태그를 클릭합니다.")
                        # pyautogui.alert("span 태그를 클릭합니다.")
                        span_element.click()
                        # time.sleep(1.5)
                    else:
...
#매물정보
        ...
        if 그룹2선택값 : new카테고리별_텍스트_찾기('item_type_box_wrap second_category_box', '2차 그룹', 'radio', 그룹2선택값)
...
#토지정보        
        ...
        #         room_number = room_info['bri_num'] if isinstance(room_info['bri_num'], bytes) else room_info['bri_num']
        #         print(room_number)
